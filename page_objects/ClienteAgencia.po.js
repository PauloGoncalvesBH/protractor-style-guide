...
class ClienteAgencia {
  constructor() {
    this.adicionarClienteAgenciaButton = element(
      by.css("a[href='/admin/cliente.php?ac=adicionar']")
    );
    ...
    this.nomeFantasiaNaoPreenchidoMessage = element(
      by.cssContainingText("p", "O campo Nome fantasia não foi preenchido.")
    );
    this.razaoSocialNaoPreenchidaMessage = element(
      by.cssContainingText("p", "O campo Razão social não foi preenchido.")
    );
  }
  ...
  cadastrarCliente({ razaoSocial = "", nomeFantasia = "", email = "" }) {
    ...
    this.preencherCamposDeCadastroClienteAgencia(
      razaoSocial,
      nomeFantasia,
      email
    );
  ...
  pesquisarClienteAgencia(NomeFantasia = "", CNPJ = "") {
    ...
    Helper.clearFieldWhenVisibleAndFillItWithText(this.pesquisaCNPJInput, CNPJ);
